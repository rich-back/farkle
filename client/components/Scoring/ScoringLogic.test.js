...
it("scores six of a kind", () => {
  let expected = countScore({
    roundScore: 0,
    rollScore: 0,
    keptDice: diceSixOfAKind,
  });
  expect(expected.roundScore).toBe(3000);
});

it("scores all variants of 1500pts", () => {
  let expected1 = countScore({
    roundScore: 0,
    rollScore: 0,
    keptDice: diceQuadPair,
  });
  let expected2 = countScore({
    roundScore: 0,
    rollScore: 0,
    keptDice: diceThreeDoubles,
  });
...
it("scores all variants of three of a kind", () => {
  let expected1 = countScore({
    roundScore: 0,
    rollScore: 0,
    keptDice: diceThreeOfAKind,
  });
  let expected2 = countScore({
    roundScore: 0,
    rollScore: 0,
    keptDice: diceThreeOfAKind2,
  });
  let expected3 = countScore({
    roundScore: 0,
    rollScore: 0,
    keptDice: diceThreeOfAKind3,
  });
  let expected4 = countScore({
    roundScore: 0,
    rollScore: 0,
    keptDice: diceThreeOfAKind4,
  });
  let expected5 = countScore({
    roundScore: 0,
    rollScore: 0,
    keptDice: diceThreeOfAKind5,
  });
  let expected6 = countScore({
    roundScore: 0,
    rollScore: 0,
    keptDice: diceThreeOfAKind6,
  });
...
it("scores single ones and/or fives", () => {
  let expected1 = countScore({
    roundScore: 0,
    rollScore: 0,
    keptDice: diceSingleOne,
  });
  let expected2 = countScore({
    roundScore: 0,
    rollScore: 0,
    keptDice: diceSingleFive,
  });
  let expected3 = countScore({
    roundScore: 0,
    rollScore: 0,
    keptDice: diceTwoOnes,
  });
  let expected4 = countScore({
    roundScore: 0,
    rollScore: 0,
    keptDice: diceTwoFives,
  });
  let expected5 = countScore({
    roundScore: 0,
    rollScore: 0,
    keptDice: diceTwoOnesTwoFives,
  });
  let expected6 = countScore({
    roundScore: 0,
    rollScore: 0,
    keptDice: diceSingleOneSingleFive,
  });
...
it("scores with less than six dice provided", () => {
  let expected = countScore({
    roundScore: 0,
    rollScore: 0,
    keptDice: fourDiceFourOfAKind,
  });
  expect(expected.roundScore).toBe(1000);
});

const singleOne = [{ key: "dice2", value: 1 }];
...
it("scores 0 for a Farkle", () => {
  let expected = countScore({
    roundScore: 0,
    rollScore: 0,
    keptDice: sixDiceFarkle,
  });
  expect(expected.roundScore).toBe(0);
});

it("ends turn - turn counter rises, overall score updates", () => {
  let expected = endTurn({ score: 20, roundScore: 10, turnCounter: 0 });
  expect(expected.score).toBe(10);
  expect(expected.turnCounter).toBe(1);
});
